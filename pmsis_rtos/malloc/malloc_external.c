...
uint32_t __malloc_extern_init(malloc_t *a, void *_chunk, int32_t size)
{
    if (size)
    {
        ...
        malloc_chunk_t *chunk = pmsis_fc_tcdm_malloc(sizeof(malloc_chunk_t));
        if (chunk == NULL)
        {
            return 1;
        }
...
{
    pmsis_fc_tcdm_malloc_free(a->first_free, sizeof(malloc_chunk_t));
}
...
void *__malloc_extern(malloc_t *a, int32_t size)
{
    ...
    if (pt)
    {
        if (pt->size == size)
        {
            ...
            pmsis_fc_tcdm_malloc_free(pt, sizeof(malloc_chunk_t));
            return addr;
        }
        else
        {
...
uint32_t __attribute__((noinline)) __malloc_extern_free(malloc_t *a, void *_chunk, int32_t size)
{
    ...
    {
        chunk = pmsis_fc_tcdm_malloc(sizeof(malloc_chunk_t));
        if (chunk == NULL)
        {
            return 1;
        }
    ...
    if (prev)
    {
        if (((int8_t *) prev->addr + prev->size) == (int8_t *) _chunk)
        {
            ...
            pmsis_fc_tcdm_malloc_free(chunk, sizeof(malloc_chunk_t));
        }
        else
        {
            prev->next = chunk;
