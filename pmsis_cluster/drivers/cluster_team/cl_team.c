...
{
    uint32_t core_mask = ((1 << nb_cores)-1);
    hal_eu_barrier_setup(core_mask);

    if(core_mask != GAP_CLUSTER_CORE0_MASK)
    {
        hal_eu_dispatch_team_config(core_mask & GAP_CLUSTER_WITHOUT_CORE0_MASK );
        hal_eu_dispatch_push((int)entry);
        hal_eu_dispatch_push((int)arg);
    }
...
{
    hal_evt_read32((uint32_t)&EU_BARRIER_DEMUX(barrier_id)->TRIGGER_WAIT_CLEAR, 0);
}
...
{
    hal_evt_read32((uint32_t)&EU_MUTEX_DEMUX->MUTEX[0], 0);
}
