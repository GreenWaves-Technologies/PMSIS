...
#ifdef __VEGA__
/*
 * Common uDMA CMD type channel enqueue
 */
static inline void udma_cmd_enqueue_channel(udma_core_cmd_t *udma_struct,
        uint32_t addr, uint32_t size, uint32_t config, udma_channel_e channel)
{
    switch(channel)
    {
        // push to rx channel
